...
# CSS Mixins
CSS Mixins offer an alternative approach to working with the Synergy Design System. They are an integral part of the systems construction, but they are not a requirement to utilize the system.
...
Mixins will only work in files that are pre-processed by PostCSS (*.post.css). If you want to use the mixins provided by Synergy you will need to import them into your PostCSS file. The path is relative so it will need to be updated based on the location of your file in the project.
...
> In order to use the mixins created by Synergy you will need to import them into your PostCSS file.

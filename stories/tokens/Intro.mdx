...
There are 5 different tiers of tokens used in the Synergy Design System. `global`, `theme`, `semantic`, `component` and `pattern` tokens. 
...
`--sds-semantic-{category}-{modifiers}`
...
`--sds-component-{category}-{modifiers}`
...
`--sds-pattern-{category}-{modifiers}`